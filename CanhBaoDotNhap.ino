...
#define BLYNK_TEMPLATE_ID "TMPL6wVIRMhi1"
#define BLYNK_TEMPLATE_NAME "IOT"

#include <BlynkSimpleEsp8266.h>
#include <SoftwareSerial.h>

SoftwareSerial SIM800(5,4); //D1-->Rx, D2-->Tx
//================AP===============//
String ssid_ap = "Long";
String pass_ap = "12345678";
String cdhd_ap = "1";
String sodt_send="0984744019";
String sms_send="";
String cdhdsim800 = "0";
boolean lockstate=0;
   ...
   <body> 
      <div id="content"> 
        <div id="homecontrol" style="height:410px; display: block">
          <div style="font-size: 30px; font-weight: bold">ALARM APP</div>
          ...
          <div>
            ...
            <button class="button_setup" onclick="configuresim800a()">SIM800A CONFIGURE</button>
          </div>
        </div>
        ...
        <div id="sim800asetup" style="height: 410px">
          <div style="font-size: 30px; font-weight: bold">ALARM APP</div>
          <div>
          ...
          <div >
            <button style="background-color: #00BB00; height: 50px; width: 135px;border-radius: 5px;color: white;font-weight: bold;margin-right: 5px" onclick="writeEEPROMsim800a()">SAVE CONFIG</button>
            <button style="background-color: blue; height: 50px; width: 135px;border-radius: 5px;color: white;font-weight: bold" onclick="test_sim800a()">TEST SIM800A</button>
          </div>
          <div>
            <input class="button_setup" type="button" onclick="backHOME()" value="BACK HOME"/>
          </div>
      ...
      <script>
        ...
        function configuresim800a(){
          ...
          for (var i = 0, length = cdhd_sim800a.length; i < length; i++) {
              if (cdhd_sim800a[i].value == cdhdsim) {
                  document.getElementsByName("cdhdsim800a")[i].checked = "true";   
                  break;
              }
          }
        }
        function writeEEPROMsim800a(){
          if(Empty(document.getElementById("sodt_send"), "Please enter mobile number!")&&Empty(document.getElementById("sms_send"), "Please enter sms content")){
            ...
            xhttp.open("GET","/writeEEPROMsim800a?sodtsend="+sodtgui+"&smssend="+smsgui+"&cdhdsim800a="+cdhd_sim800a,true);
            xhttp.onreadystatechange = process;//nhận reponse 
            xhttp.send();
          }
        }
...
void loop() {
  ...
  if(SIM800.available()){
    while(SIM800.available()){
      ...
      bufferSIM800 += inChar;
      if(bufferSIM800.length()>=160){
        bufferSIM800="";
      }
    }
    //Serial.println(bufferSIM800);
    //Khi có cuộc gọi đến thì kiểm tra số gọi đến
    Index_Rxdata = bufferSIM800.indexOf("RING");
    if(Index_Rxdata >= 0){
      Index_Rxdata = -1;
      SIM800.println("ATH");
      if(sodt_send!=""){
        Index_Rxdata = bufferSIM800.indexOf(sodt_send);
        if(Index_Rxdata >= 0){
          Index_Rxdata = -1;
          bufferSIM800="";
    ...
    Index_Rxdata = bufferSIM800.indexOf("OFF CB");
    if(Index_Rxdata >= 0){
      Index_Rxdata = -1;
      bufferSIM800="";
    ...
    Index_Rxdata = bufferSIM800.indexOf("ON CB0");
    if(Index_Rxdata >= 0){
      Index_Rxdata = -1;
      bufferSIM800="";
      ...
      canhbaostate=0;
      cdhdsim800="0";
      EEPROM.write(267, cdhdsim800[0]);
      EEPROM.write(268, lockstate);
      EEPROM.commit();
      Serial.println("Đã kích hoạt báo động chế độ Call mobile!");
    }
    Index_Rxdata = bufferSIM800.indexOf("ON CB1");
    if(Index_Rxdata >= 0){
      Index_Rxdata = -1;
      bufferSIM800="";
      Serial.println("Đã nhận tin nhắn bật cảnh báo!");
...
void read_EEPROM(){
  ...
  Serial.println("==============Thông tin cấu hình Module Sim800A===============");
  if(EEPROM.read(98)!=0){
    sodt_send="";
    sms_send="";
    cdhdsim800="";
    ...
    cdhdsim800 = char(EEPROM.read(267));
...
void webSocketEvent(uint8_t num, WStype_t type,uint8_t * payload,size_t length){
  ...
  if(payloadString == "testcall"){
    SIM800Call(sodt_send);
  }
  if(payloadString == "testsms"){
    SIM800SMS(sodt_send, sms_send);
  }
}
void updateStateD(){
  String statusD4 = "";
...
void setupSIM800A(){
  ...
  delay(1000);
  SIM800.println("AT+IPR=115200"); //Cài tốc độ baud
  delay(1000);
...
ICACHE_RAM_ATTR void Interrupt_Sensor(){
  if(lockstate==1){
